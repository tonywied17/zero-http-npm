// Modern demo UI script for uploads and management
...
uploadForm.addEventListener('submit', (e) =>
{
    ...
    // disable form while uploading to avoid duplicate actions
    const controls = uploadForm.querySelectorAll('button,input,select,textarea');
    controls.forEach(c => c.disabled = true);
    // reset to first page so new uploads appear predictably
    currentPage = 1;
    ...
    xhr.onload = () =>
    {
        ...
        } catch (e) { uploadResult.textContent = xhr.responseText; }
        // re-enable form controls
        controls.forEach(c => c.disabled = false);
        // run syntax highlighting for any JSON/code placed into the result
...
if (fileInput)
{
    fileInput.addEventListener('change', () =>
    {
        ...
        if (f) uploadResult.innerHTML = `<div class="mono">Selected: ${escapeHtml(f.name)} (${formatBytes(f.size)})</div>`;
    });
}
...
{
    ['dragenter', 'dragover'].forEach(ev => fileDrop.addEventListener(ev, (e) => { e.preventDefault(); fileDrop.classList.add('dragover'); e.dataTransfer.dropEffect = 'copy'; }));
    ['dragleave', 'dragend', 'mouseout'].forEach(ev => fileDrop.addEventListener(ev, (e) => { fileDrop.classList.remove('dragover'); }));
    fileDrop.addEventListener('drop', (e) =>
    {
        e.preventDefault(); fileDrop.classList.remove('dragover');
        const dt = e.dataTransfer;
        if (!dt || !dt.files || dt.files.length === 0) return;
        // set files to input and show filename
        try { fileInput.files = dt.files; } catch (err)
        {
            // some browsers don't allow programmatic assignment; fallback to show only
        }
        const f = dt.files[0];
        if (f) uploadResult.innerHTML = `<div class="mono">Dropped: ${escapeHtml(f.name)} (${formatBytes(f.size)})</div>`;
    });
}
...
async function loadUploadsList()
{
    try
    {
        ...
        {
            // hide internal thumbnail folder entries from the UI
            if (f.name === '.thumbs') continue;
            const card = document.createElement('div');
            card.className = 'file-card';
            // thumb or full file
            const img = document.createElement('img');
            // prefer generated thumbnail; fall back to a small placeholder for non-image files
            ...
            del.addEventListener('click', async () =>
            {
                ...
                try { highlightAllPre(); } catch (e) { }
                showUndo(f.name);
                // remove the card from the uploads list immediately
                try { card.remove(); } catch (e) { }
                // optimistically add to trash list without waiting for a full reload
                addTrashRow(f.name);
                // still refresh server-side lists in background and re-highlight any JSON
                loadUploadsList();
        ...
        // run syntax highlighting for any generated code blocks
        try { highlightAllPre(); } catch (e) { }
...
if (proxyForm)
{
    proxyForm.addEventListener('submit', async (e) =>
    {
        ...
        try
        {
            ...
            {
                // text or other — show as text blob
...
// (highlighting is run once from the main load handler)
