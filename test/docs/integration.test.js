...
describe('Docs - Body Parsers (extended features)', () => {
	...
	beforeAll(async () => {
		...
		// --- Doc: json verify callback for webhook signature verification ---
		app.post('/webhook-json', json({
			...
			res.json({ received: req.body, hasRawBody: Buffer.isBuffer(req.rawBody) });
		});

		// --- Doc: json inflate:false rejects compressed ---
		app.post('/json-no-inflate', json({ inflate: false }), (req, res) => {
			res.json({ received: req.body });
		});

		// --- Doc: urlencoded with parameterLimit and depth ---
		app.post('/form-limited', urlencoded({
			extended: true,
			parameterLimit: 5,
			depth: 3,
		}), (req, res) => {
		...
		// --- Doc: urlencoded verify callback ---
		app.post('/form-verify', urlencoded({
			verify: (req, res, buf) => {
				if (!req.headers['x-form-token']) throw new Error('Missing token');
			}
		}), (req, res) => res.json(req.body));

		// --- Doc: text verify callback + charset auto-detection ---
		app.post('/text-verify', text({
			verify: (req, res, buf, encoding) => {
				if (!req.headers['x-text-sig']) throw new Error('No sig');
			}
		}), (req, res) => {
			res.json({ body: req.body, hasRawBody: Buffer.isBuffer(req.rawBody) });
		});

		// --- Doc: raw verify callback ---
		app.post('/raw-verify', raw({
			...
			res.json({ size: req.body.length, hasRawBody: Buffer.isBuffer(req.rawBody) });
		});

		// --- Doc: multipart with limits ---
		uploadsDir = path.join(__dirname, 'tmp-doc-extended-uploads');
		app.post('/upload-limited', multipart({
			dir: uploadsDir,
			maxFiles: 2,
			maxFields: 3,
			maxFieldSize: 64,
	...
	// --- JSON verify + rawBody ---
	it('json verify: accepts valid signatures', async () => {
		const crypto = require('crypto');
		const payload = JSON.stringify({ event: 'test' });
		const sig = crypto.createHmac('sha256', 'test-secret')
			.update(Buffer.from(payload)).digest('hex');
		const r = await doFetch(`${base}/webhook-json`, {
			method: 'POST',
			headers: { 'content-type': 'application/json', 'x-signature': sig },
			body: payload,
		});
		expect(r.status).toBe(200);
		expect(r.data.received.event).toBe('test');
		expect(r.data.hasRawBody).toBe(true);
	});

	it('json verify: rejects invalid signatures with 403', async () => {
		const r = await doFetch(`${base}/webhook-json`, {
			method: 'POST',
	...
	it('json type array: matches vendor content types', async () => {
		const crypto = require('crypto');
		const payload = JSON.stringify({ data: 'vendor' });
		const sig = crypto.createHmac('sha256', 'test-secret')
			.update(Buffer.from(payload)).digest('hex');
		const r = await doFetch(`${base}/webhook-json`, {
			method: 'POST',
			headers: { 'content-type': 'application/vnd.api+json', 'x-signature': sig },
			body: payload,
		});
		expect(r.status).toBe(200);
		expect(r.data.received.data).toBe('vendor');
	});

	it('json inflate:false rejects compressed bodies', async () => {
		const zlib = require('zlib');
		const payload = JSON.stringify({ test: true });
		const compressed = zlib.gzipSync(Buffer.from(payload));
		const r = await doFetch(`${base}/json-no-inflate`, {
			method: 'POST',
	...
	it('json inflate:false accepts uncompressed bodies', async () => {
		const r = await doFetch(`${base}/json-no-inflate`, {
			method: 'POST',
			headers: { 'content-type': 'application/json' },
		...
		expect(r.status).toBe(200);
		expect(r.data.received.test).toBe(true);
	});

	// --- URLEncoded parameterLimit + depth ---
	it('urlencoded parameterLimit: accepts within limit', async () => {
		const r = await doFetch(`${base}/form-limited`, {
			method: 'POST',
			headers: { 'content-type': 'application/x-www-form-urlencoded' },
			body: 'a=1&b=2&c=3',
		});
	...
	it('urlencoded parameterLimit: rejects too many params with 413', async () => {
		const r = await doFetch(`${base}/form-limited`, {
			method: 'POST',
			headers: { 'content-type': 'application/x-www-form-urlencoded' },
	...
	it('urlencoded depth: rejects excessive nesting with 400', async () => {
		const r = await doFetch(`${base}/form-limited`, {
			method: 'POST',
			headers: { 'content-type': 'application/x-www-form-urlencoded' },
			body: 'a[b][c][d][e]=deep',
	...
	it('urlencoded verify: rejects missing token with 403', async () => {
		const r = await doFetch(`${base}/form-verify`, {
			method: 'POST',
			headers: { 'content-type': 'application/x-www-form-urlencoded' },
			body: 'foo=bar',
		});
	...
	it('urlencoded verify: accepts valid token', async () => {
		const r = await doFetch(`${base}/form-verify`, {
			method: 'POST',
			headers: { 'content-type': 'application/x-www-form-urlencoded', 'x-form-token': 'valid' },
			body: 'foo=bar',
		});
		expect(r.status).toBe(200);
	...
	// --- Text verify + rawBody ---
	it('text verify: rejects missing sig with 403', async () => {
		const r = await doFetch(`${base}/text-verify`, {
			method: 'POST',
			headers: { 'content-type': 'text/plain' },
			body: 'hello',
		});
	...
	it('text verify: accepts with sig + rawBody', async () => {
		const r = await doFetch(`${base}/text-verify`, {
			method: 'POST',
			headers: { 'content-type': 'text/plain', 'x-text-sig': 'present' },
			body: 'hello',
		});
		expect(r.status).toBe(200);
	...
	// --- Raw verify + rawBody ---
	it('raw verify: rejects empty body with 403', async () => {
		const r = await doFetch(`${base}/raw-verify`, {
			method: 'POST',
	...
	it('raw verify: accepts non-empty body + rawBody', async () => {
		const r = await doFetch(`${base}/raw-verify`, {
			method: 'POST',
			headers: { 'content-type': 'application/octet-stream' },
		...
		expect(r.status).toBe(200);
		expect(r.data.size).toBe(3);
		expect(r.data.hasRawBody).toBe(true);
	});

	// --- Multipart upload limits ---
	it('multipart maxFiles: rejects too many files with 413', async () => {
		const boundary = '----TestBound' + Date.now();
		const parts = [];
		for (let i = 0; i < 3; i++) {
			parts.push(`--${boundary}`);
			parts.push(`Content-Disposition: form-data; name="file${i}"; filename="f${i}.txt"`);
			parts.push('Content-Type: text/plain');
			parts.push('');
			parts.push('data');
		}
		parts.push(`--${boundary}--`);
		const r = await doFetch(`${base}/upload-limited`, {
			method: 'POST',
			headers: { 'content-type': `multipart/form-data; boundary=${boundary}` },
			body: parts.join('\r\n'),
		});
	...
	it('multipart maxFields: rejects too many fields with 413', async () => {
		const boundary = '----TestBound' + Date.now();
		const parts = [];
		for (let i = 0; i < 4; i++) {
			parts.push(`--${boundary}`);
	...
	it('multipart allowedMimeTypes: rejects disallowed types with 415', async () => {
		const boundary = '----TestBound' + Date.now();
		const body = [
			`--${boundary}`,
	...
	it('multipart allowedMimeTypes: accepts allowed types', async () => {
		const boundary = '----TestBound' + Date.now();
		const body = [
			`--${boundary}`,
	...
	it('multipart maxFieldSize: rejects oversized field values with 413', async () => {
		const boundary = '----TestBound' + Date.now();
...
describe('Docs - ORM: QueryProfiler', () => {
	...
	it('enableProfiling attaches a profiler and returns it', () => {
	...
	it('onSlow callback fires for slow queries', () => {
		...
		profiler.record({ table: 'test', action: 'select', duration: 5 });
		expect(slow.length).toBe(1);
		expect(slow[0].duration).toBe(5);
	});

	it('N+1 detection fires onN1 callback', () => {
		const hits = [];
		const profiler = db.enableProfiling({
			n1Threshold: 3,
			n1Window: 5000,
			onN1: (info) => hits.push(info),
		});
		for (let i = 0; i < 4; i++) {
			profiler.record({ table: 'comments', action: 'select', duration: 1 });
		}
		expect(hits.length).toBe(1);
		expect(hits[0].table).toBe('comments');
		expect(hits[0].message).toContain('N+1');
	});

	it('getQueries filters by table and action', () => {
		const profiler = db.enableProfiling();
		profiler.record({ table: 'users', action: 'select', duration: 1 });
		profiler.record({ table: 'posts', action: 'insert', duration: 2 });
		expect(profiler.getQueries({ table: 'users' })).toHaveLength(1);
		expect(profiler.getQueries({ action: 'insert' })).toHaveLength(1);
	});
	...
	it('enabled can be toggled at runtime', () => {
		...
		profiler.record({ table: 'x', action: 'select', duration: 1 });
		expect(profiler.getQueries()).toHaveLength(0);
		profiler.enabled = true;
		profiler.record({ table: 'x', action: 'select', duration: 1 });
		expect(profiler.getQueries()).toHaveLength(1);
	});
});
...
describe('Docs - ORM: ReplicaManager', () => {
	...
	it('manual setup with setPrimary and addReplica', () => {
		const mgr = new ReplicaManager({ strategy: 'random' });
		const primary = Database.connect('memory');
		mgr.setPrimary(primary.adapter);

		const r1 = Database.connect('memory');
		const r2 = Database.connect('memory');
		mgr.addReplica(r1.adapter);
		mgr.addReplica(r2.adapter);

		expect(mgr.status().total).toBe(2);
		expect(mgr.status().primary).toBe(true);
	});

	it('getReadAdapter distributes reads (round-robin)', () => {
		const mgr = new ReplicaManager({ strategy: 'round-robin' });
		const p = Database.connect('memory');
		const r1 = Database.connect('memory');
		const r2 = Database.connect('memory');
		mgr.setPrimary(p.adapter);
		mgr.addReplica(r1.adapter);
		mgr.addReplica(r2.adapter);

		const a1 = mgr.getReadAdapter();
		const a2 = mgr.getReadAdapter();
		expect(a1).not.toBe(a2);
	});

	it('getWriteAdapter returns primary and triggers sticky', () => {
		...
		const writer = mgr.getWriteAdapter();
		expect(writer).toBe(p.adapter);

		// After a write, reads go to primary (sticky)
		const reader = mgr.getReadAdapter();
		expect(reader).toBe(p.adapter);
	});

	it('markUnhealthy/markHealthy toggles availability', () => {
		...
		mgr.markUnhealthy(r1.adapter);
		expect(mgr.status().unhealthy).toBe(1);
		// Falls back to primary
		expect(mgr.getReadAdapter()).toBe(p.adapter);

		mgr.markHealthy(r1.adapter);
		expect(mgr.status().unhealthy).toBe(0);
	});

	it('healthCheck auto-detects down replicas', async () => {
		const mgr = new ReplicaManager();
		const p = Database.connect('memory');
		mgr.setPrimary(p.adapter);

		const fakeDown = { ping: async () => { throw new Error('down'); } };
		mgr.addReplica(fakeDown);

		await mgr.healthCheck();
		expect(mgr.status().unhealthy).toBe(1);
	});

	it('strategy validation rejects invalid values', () => {
		expect(() => new ReplicaManager({ strategy: 'fastest' })).toThrow(/Invalid replica strategy/);
	});

	it('connectWithReplicas rejects non-array replicaConfigs', () => {
		expect(() => Database.connectWithReplicas('memory', {}, 'not-array')).toThrow(/must be an array/);
	});
});
