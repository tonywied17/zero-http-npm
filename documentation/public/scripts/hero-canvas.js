 ...
 * Floating orbs drift independently — no connecting lines —
 * representing self-contained, dependency-free architecture.
 * Respects the pause FAB and adapts colors to theme.
 */
(function () {
    ...
    /* Listen for the global pause FAB toggle */
    window.addEventListener('waves-pause', () => { stop(); });
    window.addEventListener('waves-resume', () => { start(); });

    /* Also watch localStorage for the pause toggle (matches background.js behaviour) */
    ...
    /* Observe theme changes so colors adapt */
