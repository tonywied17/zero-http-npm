...
function getAllMethods(item) {
    return [
        ...(item.methods || []),
        ...(item.methodGroups || []).flatMap(g => g.methods || []),
    ];
}
...
describe('Documentation Examples', () => {
    ...
    it('docs sections load and have content', () => {
    ...
    describe('Documented methods exist on exports', () => {
        for (const section of docs) {
            for (const item of section.items) {
                if (!getAllMethods(item).length) continue;
                ...
                it(`[${section.section}] ${item.name} — documented methods exist`, () => {
                    ...
                    for (const methodDef of getAllMethods(item)) {
    ...
    describe('Error Classes', () => {
        ...
        it('all documented error classes are exported', () => {
            ...
            const missing = [];
            for (const methodDef of getAllMethods(errorClassItem)) {
    ...
    describe('env module API', () => {
        ...
        if (!envItem || !getAllMethods(envItem).length) return;

        it('all documented env methods exist', () => {
            const missing = [];
            for (const methodDef of getAllMethods(envItem)) {
                const name = methodDef.method;
    ...
    describe('WebSocketPool API', () => {
        ...
        if (!poolItem || !getAllMethods(poolItem).length) return;

        it('all documented WebSocketPool methods exist', () => {
            const pool = new zeroHttp.WebSocketPool();
            const missing = [];
            for (const methodDef of getAllMethods(poolItem)) {
                const name = methodDef.method;
    ...
    describe('Database docs completeness', () => {
        ...
        it('has all migration methods documented', () => {
            const documented = getAllMethods(dbItem).map(m => m.method);
            const expected = [
                'addColumn', 'dropColumn', 'renameColumn', 'renameTable',
                'createIndex', 'dropIndex', 'hasTable', 'hasColumn',
                'describeTable', 'addForeignKey', 'dropForeignKey',
        ...
        it('dropIndex signature uses (table, name)', () => {
            const di = getAllMethods(dbItem).find(m => m.method === 'dropIndex');
