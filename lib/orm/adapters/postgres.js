...
class PostgresAdapter extends BaseSqlAdapter
{
        ...
        for (const [name, def] of Object.entries(schema))
        {
            ...
            else
            {
                ...
                if (def.references)
                {
                    ...
                    if (def.references.onUpdate) line += ` ON UPDATE ${def.references.onUpdate}`;
                }
            }
            cols.push(line);
        }
        let sql = `CREATE TABLE IF NOT EXISTS "${table}" (${cols.join(', ')})`;
    ...
    // -- Table Info & Debug (PHPMyAdmin-style) -----------
    ...
    /**
     * Get table and column comments (like PHPMyAdmin's table info).
     * @param {string} table
